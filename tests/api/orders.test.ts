...
vi.mock('@/lib/format', () => ({
  normalizeSocialHandle: vi.fn((platform: string, handle: string) => handle),
  formatPrice: vi.fn((n: number) => n + '.00'),
  maskSocialHandle: vi.fn((platform: string, handle: string) => handle),
...
describe('POST /api/orders', () => {
  beforeEach(() => {
    ...
    ;(normalizeSocialHandle as any).mockReturnValue('test-handle')
  ...
  it('returns 404 when note is not found', async () => {
    ...
    req.json = () => Promise.resolve({ itemType: 'note', itemSlug: 'nonexistent', fullName: 'Test', socialPlatform: 'email', socialHandle: 'test@test.com', consentAccepted: true })
    const result = await ordersPOST(req as any, undefined)
    expect(result).toBeDefined()
  })
  ...
  it('returns 404 when group is not found', async () => {
    ...
    req.json = () => Promise.resolve({ itemType: 'group', itemSlug: 'nonexistent', fullName: 'Test', socialPlatform: 'email', socialHandle: 'test@test.com', consentAccepted: true })
    const result = await ordersPOST(req as any, undefined)
    expect(result).toBeDefined()
  })
  ...
  it('throws validation error for free notes', async () => {
    ...
    req.json = () => Promise.resolve({ itemType: 'note', itemSlug: 'free-note', fullName: 'Test', socialPlatform: 'email', socialHandle: 'test@test.com', consentAccepted: true })
    const result = await ordersPOST(req as any, undefined)
    expect(result).toBeDefined()
  })
  ...
  it('throws validation error for free groups', async () => {
    ...
    req.json = () => Promise.resolve({ itemType: 'group', itemSlug: 'free-group', fullName: 'Test', socialPlatform: 'email', socialHandle: 'test@test.com', consentAccepted: true })
    const result = await ordersPOST(req as any, undefined)
    expect(result).toBeDefined()
  })
  ...
  it('throws validation error for amounts below minimum', async () => {
    ...
    req.json = () => Promise.resolve({ itemType: 'note', itemSlug: 'cheap-note', fullName: 'Test', socialPlatform: 'email', socialHandle: 'test@test.com', consentAccepted: true })
    const result = await ordersPOST(req as any, undefined)
    expect(result).toBeDefined()
  })
  ...
  it('creates order successfully for paid note', async () => {
    ...
    const req = new NextRequest('http://localhost/api/orders', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ itemType: 'note', itemSlug: 'paid-note', fullName: 'Test User', socialPlatform: 'email', socialHandle: 'test@test.com', consentAccepted: true }),
    })
    req.json = () => Promise.resolve({ itemType: 'note', itemSlug: 'paid-note', fullName: 'Test User', socialPlatform: 'email', socialHandle: 'test@test.com', consentAccepted: true })
    const result = await ordersPOST(req as any, undefined)
    expect(result).toBeDefined()
  })
})
