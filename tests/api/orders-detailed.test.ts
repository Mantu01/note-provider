...
vi.mock('@/lib/format', () => ({
  ...
  isValidSocialHandle: vi.fn(() => true),
  toIsoStringRequired: vi.fn((v: string) => v),
  toIsoString: vi.fn((v: any) => v),
...
describe('POST /api/orders', () => {
  ...
  it('returns validation error for invalid social platform', async () => {
    const req = new NextRequest('http://localhost/api/orders', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ ...VALID_BODY, socialPlatform: 'twitter' as any }),
    })
    req.json = () => Promise.resolve({ ...VALID_BODY, socialPlatform: 'twitter' })
    const result = await ordersPOST(req as any, undefined)
    expect(result.status).toBe(400)
  })
  ...
  it('normalizes social handle before passing to createOrder', async () => {
    ;(Note.findOne as any).mockReturnValue(makeChain({ pricingType: 'paid', price: 500, title: 'Note' }))
    ;(normalizeSocialHandle as any).mockReturnValue('normalized-handle')
    const req = new NextRequest('http://localhost/api/orders', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    expect(normalizeSocialHandle).toHaveBeenCalledWith('email', 'test@test.com')
    expect(createOrder).toHaveBeenCalledWith(
      expect.objectContaining({ socialHandle: 'normalized-handle' }),
      'n',
      'note',
      500,
      expect.anything(),
