...
type PostgresUniqueViolationError = { code: string; detail?: string };

export function isDuplicateKeyError(error: unknown): error is PostgresUniqueViolationError {
  return (
    typeof error === "object" &&
    error !== null &&
    "code" in error &&
...
export function duplicateKeyToAppError(error: PostgresUniqueViolationError): AppError {
