...
export function GroupCard({ group }: { group: PublicGroup }) {
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-lg border border-border bg-card transition-shadow hover:shadow-md max-sm:flex-row">
      <Link
        href={`/groups/${group.slug}`}
        tabIndex={-1}
        aria-hidden="true"
        className="relative aspect-[16/9] w-full shrink-0 overflow-hidden bg-muted/20 max-sm:aspect-auto max-sm:w-24"
      >
        {group.coverImageUrl ? (
          <Image
        ...
        <span className="absolute right-1.5 bottom-1.5 inline-flex items-center gap-1 rounded-full bg-card/95 px-1.5 py-0.5 text-[9px] font-semibold text-foreground shadow-sm">
