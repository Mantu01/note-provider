    ...
    <section className="relative overflow-hidden pt-8 pb-8 md:pt-16 md:pb-12 lg:pt-24 lg:pb-16 paper-bg">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl space-y-6">
          ...
          <h1 className="font-heading text-3xl font-extrabold tracking-tighter leading-[1.1] text-foreground md:text-4xl lg:text-5xl xl:text-6xl animate-fade-in-up stagger-1">
            Learn the stack{" "}
          ...
          <p className="max-w-lg text-sm leading-relaxed text-muted-foreground animate-fade-in-up stagger-2">
            {BRAND.description}
          </p>

          <div className="flex flex-wrap gap-2.5 pt-1 animate-fade-in-up stagger-3">
            <Button
            ...
            <Button
              ...
              variant="outline"
              size="lg"
              className="h-11 rounded-full border-2 px-6 text-sm font-semibold backdrop-blur-sm hover:bg-primary/5 transition-colors"
            >
              Free notes
            </Button>
          </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-2.5 md:mt-14 md:grid-cols-4 animate-fade-in-up stagger-4">
          {isLoading
            ...
            : HOME_STATS_CONFIG.map((s, i) => (
                <div
                  key={s.key}
                  className="animate-fade-in-up rounded-xl border border-border/60 bg-card px-3 py-3 text-center"
    ...
    <div className="flex gap-2.5 overflow-x-auto pb-3 hide-scrollbar">
      {isLoading ? (
        Array.from({ length: 5 }, (_, i) => (
          <ShimmerLoader key={i} className="min-w-36 h-14 rounded-xl shrink-0" />
        ))
      ) : categories?.length ? (
...
function StepsRow() {
  return (
    <div className="relative">
      <div className="absolute top-7 left-0 right-0 hidden border-t-2 border-dashed border-border/60 lg:block" />

      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4 lg:gap-3">
        {HOME_STEPS.map(({ num, title, desc, Icon }, index) => (
          <div key={num} className="relative flex flex-col items-center text-center">
            <div className="relative z-10 mb-3 flex size-14 items-center justify-center rounded-2xl border border-border/60 bg-card transition-shadow hover:shadow-md">
              <Icon aria-hidden="true" className="size-5 text-primary" />
            </div>

            <span className="mb-1 inline-flex size-5 items-center justify-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground">
              {num}
            </span>
          ...
          <div className="relative z-10 mx-auto max-w-lg space-y-5">
            ...
            <div className="flex flex-wrap justify-center gap-2.5 pt-1">
              ...
              <Button
                ...
                variant="outline"
                size="lg"
                className="h-11 rounded-full px-6 text-sm font-semibold backdrop-blur-sm hover:bg-primary/5 transition-colors"
              >
                Contact support
              </Button>
            </div>
...
function FAQAccordion() {
  return (
    <Accordion defaultValue={["faq-0"]} className="rounded-xl border border-border bg-card">
      {HOME_FAQS.map(([q, a], i) => (
        <AccordionItem key={q} value={`faq-${i}`} className="border-b-0">
          <AccordionTrigger className="px-4 py-3 text-sm font-semibold sm:px-5 sm:py-4">
            {q}
          </AccordionTrigger>
          <AccordionContent className="px-4 pb-3 text-sm leading-relaxed text-muted-foreground sm:px-5 sm:pb-4">
            {a}
          </AccordionContent>
        </AccordionItem>
      ))}
