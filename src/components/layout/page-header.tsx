...
export function PageHeader({ eyebrow, title, description, action, className }: { eyebrow?: string; title: string; description?: string; action?: ReactNode; className?: string }) {
  return <header className={cn("flex flex-col gap-4 border-b pb-8 md:flex-row md:items-end md:justify-between", className)}>
    <div className="max-w-2xl space-y-2">
      {eyebrow && <p className="text-sm font-medium tracking-wide text-primary uppercase">{eyebrow}</p>}
